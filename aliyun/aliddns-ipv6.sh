...
AccessKeySec="yyyyyyyyyyyyy"
DomainRecordId="111111111111"
# DomainRR, use "@" to set top level domain
DomainRR="11111"
DomainName="222222.win"
DomainType="AAAA"
...
#ipv6----   ip -6 address show | grep inet6 |grep /60 | awk '{print $2}' | cut -d'/' -f1
MyIPEchoUrl=$(ip -6 address show | grep inet6 |grep /60 | awk '{print $2}' | cut -d'/' -f1)
...
get_domain_ip()
{
	...
	echo iiiiiiiiiiii${full_domain}
	_func_ret=$(nslookup  "${full_domain}"  |grep "Address 1:" |grep -v 127.0.0.1 | awk '{print $3}')
	echo ========$_func_ret
